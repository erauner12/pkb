...
[[http://www.ruanyifeng.com/blog/2016/09/software-architecture.html][Source]]
...
[[http://blog.csdn.net/mindfloating/article/details/51221780][Source]]
...
[[http://blog.csdn.net/mindfloating/article/details/50166169][Source]]
...
[[http://highscalability.com/blog/2013/5/20/the-tumblr-architecture-yahoo-bought-for-a-cool-billion-doll.html][Source]]
...
[[http://www.ruanyifeng.com/blog/2009/08/what_is_web_service.html][Source]]

下一代互联网软件将建立在 Web service（也就是「云」）的基础上。

** Web service 的概念

想要理解 Web service，必须先理解什么是 service（服务）。

传统上，我们把计算机后台程序（daemon）提供的功能，称为「服务」（service）。比如，一个杀毒软件在后台运行，它会自动监控系统，这种自动监控就是一个「服务」。通俗地说，「服务」就是计算机可以提供的某种功能。

根据来源的不同，「服务」又可以分成两种：「本地服务」（使用同一台机器提供的服务，不需要网络），和「网络服务」（使用另一台计算机提供的服务，必须通过网络才能完成）。

举例来说，现在有一批图片，需要把它们的大小缩小一半。把「缩放图片」看成是一种服务。可以使用「本地服务」，在自己计算机上用软件缩小图片，也可以使用「网络服务」，将图片上传到某个网站，让服务器缩小图片，完成后再通过网络送回。

_Web Service 的本质，就是通过网络调用其他网站的资源。_

举例来说，去年我写过一个「四川大地震图片墙」，它能动态显示关于四川地震的最新图片。但是，所有的图片都不是储存在我的服务器上，而是来自 Flickr。我只是发出一个动态请求，要求 Flickr 提供图片。这种情况下，Flickr 提供的就是一种 Web service。

Web service 让你的网站可以使用其他网站的资源，比如在网页上显示天气、地图、Twitter 上的最新动态等等。
...
[[http://www.lecloud.net/tagged/scalability][Source]]
...
But what about deployment? How can you make sure that a code change is sent to all your servers without one server still serving old code? This tricky problem is fortunately already solved by the great tool _Capistrano_.

Create an image file from one of these servers. AWS calls this AMI - Amazon Machine Image. Use this AMI as a "super-clone" that all new instances are based upon. For a new instance, just do an initial deployment of latest code and it is ready.
...
Path 2 is to _denormalize right from the beginning and include no more =join= in any database query._ You can stay with MySQL, and use it like a NoSQL database, or you can switch to a better and easier to scale NoSQL database like MongoDB or CouchDB. _Joins will now need to be done in application code._

** Part 3: Cache

With a scalable database solution, you have no fear of storing terabytes. But users still have to suffer slow page requests when a lot of data is fetched from the database. The solution is the implementation of a cache.

With "cache" I always mean _in-memory caches like Memcached or Redis. Never do file-based caching, it makes cloning and auto-scaling of your servers just a pain._
...
Whenever you do a query to database, store the result dataset in cache. A hashed version of the query is the cache key. The next time you run the query, first check if it is already in the cache. This pattern has several issues. The main issue is the expiration. _It is hard to delete a cached result when you cache a complex query. When one piece of data changes, you need to delete all cached queries who may include that table cell._
...
In general, I am more a friend of Redis than Memcached, because I love the extra database-features of Redis like persistence and the built-in data structures like lists and sets. But if you just need to cache, take Memcached, because it scales like a charm.

** Part 4: Asynchronism

Imagine that you want to buy bread at a bakery. But there is no bread at the moment, instead you are asked to come back in 2 hours. To avoid such a "please wait" situation, asynchronism needs to be done. In general, there are two paradigms:
...
_RabbitMQ_ is one of many systems which help to implement async processing. You could also use ActiveMQ or a simple Redis list.
